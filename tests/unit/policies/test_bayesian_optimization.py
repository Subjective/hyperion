...
class MockReadableState:
    ...
    def trial(self, trial_id: str) -> TrialView | None:  # pragma: no cover - unused
        return None

    def running_trials(
        self, experiment_id: str | None = None
    ) -> list[TrialView]:  # pragma: no cover - unused
        return []
    ...
    def trials_by_depth(
        self, experiment_id: str, depth: int
    ) -> list[TrialView]:  # pragma: no cover - unused
        return []
...
@pytest.mark.asyncio
async def test_bayesian_uses_guided_after_initial():
    """BayesianOptimizationPolicy should use guided search after initial exploration."""
    space = {"x": Float(0.0, 1.0), "y": Choice(["option_a", "option_b"])}
    policy = BayesianOptimizationPolicy(space=space, n_initial=3, experiment_id="exp-1")

    # With enough completed trials, should use acquisition-like selection
...
@pytest.mark.asyncio
async def test_bayesian_handles_various_param_types():
    space = {
        "x": Float(0.0, 1.0),
        "n": Int(1, 5),
        "cat": Choice(["a", "b", "c"]),
    ...
    policy = BayesianOptimizationPolicy(space=space, n_initial=2, experiment_id="exp-1")
    # Provide a few completed trials via count mock
    state = MockReadableState(completed_count=5)

    actions = await policy.decide(state)
    assert len(actions) > 0
    for a in actions:
        assert isinstance(a, StartTrial)
        assert 0.0 <= a.params["x"] <= 1.0
        assert 1 <= a.params["n"] <= 5
        assert a.params["cat"] in ["a", "b", "c"]
        assert isinstance(a.params["flag"], bool)
...
async def test_bayesian_tracks_events():
    ...
    # Rationale should reflect TPE and warmup
    r = await policy.rationale()
    assert r and "tpe" in r.lower()
